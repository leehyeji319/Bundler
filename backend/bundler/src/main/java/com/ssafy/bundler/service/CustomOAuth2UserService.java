...
public class CustomOAuth2UserService extends DefaultOAuth2UserService {
	...
	public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
		...
		////////////////////////
		log.debug(CustomOAuth2UserService.class + "- loadUser 의 userRequest: " + userRequest);
		////////////////////////
	...
	private OAuth2User process(OAuth2UserRequest userRequest, OAuth2User oAuthUser) throws
		...
		// if (principal != null) { //로그인 된 상태면
		if (authentication.isAuthenticated()) { //로그인 된 상태면
			log.debug("로그인 된 상태");
			...
			if (isOAuthUserExists) { //기존 OAuth 계정이 있었다면
				log.debug("기존 OAuth 계정이 있었다면, OAuth 로그인");
				...
				log.debug("기존 OAuth 계정이 없었다면, OAuth 회원가입");
